...
export default function PiezasPage() {
  ...
  async function cargarDatos() {
    const { data: piezasData } = await supabase
      .from('piezas')
      .select('*, usuarios!piezas_alumno_id_fkey(nombre, apellido), estados!piezas_estado_actual_id_fkey(nombre), flujos(nombre)')
  ...
  async function handleAvanzarEstado(pieza: any) {
    ...
    await supabase
      .from('piezas')
      .update({
        estado_actual_id: siguienteEstado.id,
        finalizada: esFinal,
      })
      .eq('id', pieza.id)
  ...
  function actualizarEtapa(index: number, valor: string) {
    const nuevas = [...etapas]
    nuevas[index] = valor
    setEtapas(nuevas)
  }

  function eliminarEtapa(index: number) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          {vista === 'piezas' ? (
            <button onClick={() => setMostrarFormPieza(true)} className="bg-naranja-500 hover:bg-naranja-600 text-white rounded-lg px-4 py-2.5 text-sm font-medium transition-colors">+ Nueva pieza</button>
          ) : (
            <button onClick={() => setMostrarFormFlujo(true)} className="bg-naranja-500 hover:bg-naranja-600 text-white rounded-lg px-4 py-2.5 text-sm font-medium transition-colors">+ Nuevo flujo</button>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setMostrarFormFlujo(false); setError(''); setEtapas(['']) }} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-3 mt-2">
                <button onClick={() => { setMostrarFormFlujo(false); setError(''); setEtapas(['']) }} className="flex-1 border border-gray-200 text-gray-600 rounded-lg py-2.5 text-sm font-medium hover:bg-gray-50 transition-colors">Cancelar</button>
    ...
    {piezaAvanzar && (() => {
      ...
      {vista === 'flujos' ? (
        <div>
          {flujos.length === 0 ? (
            <p className="text-gray-400 text-sm">Aún no hay flujos creados. Creá uno para empezar a registrar piezas.</p>
          ) : (
